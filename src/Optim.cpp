...
void Optim::constraintImages(Patch::Cpatch& patch, const float nccThreshold, const int id) {
...
int Optim::grabSafe(const int index, const int size, const Vec3f& center, const Vec3f& dx, const Vec3f& dy,
...
int Optim::grabTex(const Vec4f& coord,
                   const Vec4f& pxaxis,
                   const Vec4f& pyaxis,
                   const Vec4f& pzaxis,
                   const int index,
    ...
    // Upper limit is 2
    leveldif = max(-m_df.getmLevel(), min(2, leveldif));
...
double Optim::my_f(unsigned n, const double* x, double* grad, void* my_func_data) {
    ...
    const int mininum = min(m_one->m_df.m_minimagenumthresho, size);
...
bool Optim::refinePatchBFGS(Patch::Cpatch& patch, const int id, const int time, const int ncc) {
    int idtmp = id;
...
void Optim::computeUnits(const Patch::Cpatch& patch, std::vector<float>& units) const {
...
void Optim::refinePatch(Patch::Cpatch& patch, const int id, const int time) {
...
double Optim::computeINCC(const Vec4f& coord, const Vec4f& normal, const std::vector<int>& indexes, const int id,
                          const int robust) {
...
double Optim::computeINCC(const Vec4f& coord,
                          const Vec4f& normal,
                          const std::vector<int>& indexes,
                          const Vec4f& pxaxis,
                          const Vec4f& pyaxis,
