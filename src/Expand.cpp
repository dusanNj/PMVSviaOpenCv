...
void Expand::run(void) {
    ...
    m_df.m_pos.collectPatches(m_queue);
    ...
    while (1) {
        ...
        for (int i = 0; i < (int)canCoords.size(); ++i) {
            for (int j = 0; j < (int)canCoords[i].size(); ++j) {
                const int flag = expandSub(ppatch, 0, canCoords[i][j]);
                // fail
                if (flag) {
...
void Expand::findEmptyBlocks(const Patch::Ppatch& ppatch, std::vector<std::vector<Vec4f> >& canCoords) {
...
float Expand::computeRadius(const Patch::Cpatch& patch) {
    ...
    m_df.m_optim.computeUnits(patch, units);
    ...
    // ??? critical
    return (*(vftmp.begin() + minnum - 1)) * m_df.getmCsize();
}

int Expand::expandSub(const Patch::Ppatch& orgppatch, const int id, const Vec4f& canCoord) {
    ...
    m_df.m_pos.setGridsImages(patch, orgppatch->m_images);
    ...
    // -----------------------------------------------------------------
    // Preprocess
    if (m_df.m_optim.preProcess(patch, id, 0)) {
        ++m_fcounts0[id];
        return 1;
    }
    ...
    // -----------------------------------------------------------------
    if (m_df.m_optim.postProcess(patch, id, 0)) {
        ++m_fcounts1[id];
        return 1;
    }
    ++m_pcounts[id];
...
int Expand::checkCounts(Patch::Cpatch& patch) {
    ...
    std::vector<int>::iterator begin = patch.m_images.begin();
    std::vector<int>::iterator end = patch.m_images.end();
    std::vector<Vec2i>::iterator begin2 = patch.m_grids.begin();
    ...
    while (begin != end) {
        ...
        if ((ix < 0) || (m_df.m_pos.m_gwidths[index] <= ix) ||
            (iy < 0) || (m_df.m_pos.m_gheights[index] <= iy)) {
            ++begin;
            ++begin2;
            continue;
        }
        ...
        if (m_df.m_countThreshold1 <= m_df.m_pos.m_counts[index][index2]) {
            ++full;
        } else {
            ++empty;
        }
    ...
    if (m_df.m_depth <= 1) {
        if ((empty < m_df.m_minimagenumthresho) && (full != 0)) {
            return 1;
        } else {
            return 0;
        }
    } else {
        if ((empty < m_df.m_minimagenumthresho - 1) && (full != 0)) {
            return 1;
        } else {
            return 0;
        }
...
int Expand::updateCounts(const Patch::Cpatch& patch) {
...

    {
        ...
        std::vector<Vec2i>::const_iterator begin2 = patch.m_grids.begin();
    ...
    {
        std::vector<int>::const_iterator begin = patch.m_vimages.begin();
        std::vector<int>::const_iterator end = patch.m_vimages.end();
        std::vector<Vec2i>::const_iterator begin2 = patch.m_vgrids.begin();
