...
void Harris::init(const std::vector<unsigned char>& image,
	...
	for (int y = 0; y < m_height; ++y) {
		...
		for (int x = 0; x < m_width; ++x) { 
			...
			//count++;
			//if (((int)image[count++]) / 255.0f != 0) {
			//	std::cerr << "value:" << ((int)image[count++]) / 255.0f << std::endl;
			//}
...
void Harris::preprocess(void) {
	...
	//Drugacije staviti maska bude una nula i onda kvari konvoluciju
	m_mask.clear();
...
void Harris::preprocess2(void) {
	...
	for (int y = 0; y < m_height; ++y) {
		...
		for (int x = 0; x < m_width; ++x) {
			...
			//if (y==1040 && x == 1345) {
			//	std::cout << " m_dIdxdIdx:" << m_dIdxdIdx[y][x] << std::endl;
			//}
...
void Harris::run( std::vector<std::vector<unsigned char>>& image,
	const std::vector<unsigned char> mask,
	const std::vector<unsigned char> edge,
	...
	std::cout << "result grid size:" << resultgrids.size() << std::endl;
