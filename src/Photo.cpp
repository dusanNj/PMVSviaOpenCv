...
Vec3f Photo::project(const int index, const Vec4f& coord, const int level) const {
    return Camera::project(coord, level);
}

float Photo::computeDepth(const int index, const Vec4f& coord) {
    return Camera::computeDepth(coord);
}
